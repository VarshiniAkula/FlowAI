...
import { LlmStatusBanner, LlmPrivacyNotice } from '@/components/integrations/llm-status-banner';
...
export function Simulator({ assistantId }: Props) {
  ...
  const applyTurn = (result: RunTurnResult) => {
    ...
    setDone(result.done);
    // A completed turn may have consumed a Groq demo request — refresh the
    // remaining allowance shown in the banner.
    void llm.refresh();
  };
  ...
  return (
    <div className="flex h-full overflow-hidden bg-gradient-to-b from-background via-background to-indigo-950/5">
      ...
      <div className="flex flex-1 min-w-0 flex-col border-r">
        ...
        <LlmStatusBanner status={llm.status} />
        <LlmPrivacyNotice status={llm.status} />
